...
P1=Series_training(1:N-1,:);  
T1=Series_training(N,:);

% P1(1:(N+1)/2-1,:)=Series_training(1:(N+1)/2-1,:);  
% P1((N+1)/2:N-1,:)=Series_training((N+1)/2+1:N,:);  
...
for j=1:9
  for iter=1:M_pred
     ...
     Series_pred(iter,:)=ESource(1,iter+100000*j:iter+100000*j+N-1);
  end
  Series_pred1=Series_pred';
  P2=Series_pred1(1:N-1,:);  
% figure()
% imshow(P2(:,100:1000),[])
